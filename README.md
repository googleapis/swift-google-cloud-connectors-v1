# Google Cloud Client Libraries for Swift - Connectors API
...
Add `swift-google-cloud-connectors-v1` as a package dependency:

```bash
swift package add-dependency https://github.com/googleapis/swift-google-cloud-connectors-v1.git --from 0.1.0-preview
```

Then add `GoogleCloudConnectorsV1` to your target's dependencies:

```bash
swift package add-target-dependency GoogleCloudConnectorsV1 <target-name> --package swift-google-cloud-connectors-v1
```
